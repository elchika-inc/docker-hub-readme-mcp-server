...
export class MemoryCache {
  ...
  constructor(options: CacheOptions = {}) {
    this.defaultTtl = options.ttl || CACHE_CONFIG.TTL.DEFAULT;
    this.maxSize = options.maxSize || CACHE_CONFIG.SIZES.DEFAULT_MAX_SIZE;
...
export const createCacheKey = {
  imageInfo: (imageName: string, tag: string): string => 
    `img_info:${imageName}:${tag}`,
  
  imageReadme: (imageName: string, tag: string): string => 
    `img_readme:${imageName}:${tag}`,
  
  searchResults: (query: string, limit: number, isOfficial?: boolean, isAutomated?: boolean): string => {
    const queryHash = Buffer.from(query).toString('base64');
    const params = [queryHash, limit.toString()];
    if (isOfficial !== undefined) {params.push(`official:${isOfficial}`);}
    if (isAutomated !== undefined) {params.push(`automated:${isAutomated}`);}
    return `search:${params.join(':')}`;
  },
  
  imageTags: (imageName: string): string => 
    `tags:${imageName}:${new Date().toISOString().split('T')[0]}`, // Include date for daily invalidation
};
